...
file_handler = logging.FileHandler('first_logger.log')
...
print('====4th logger: customize formatter for multiple parameters====')
...
fourth_logger = logging.getLogger('4th logger')
console_output_handler_with_multiple_params = logging.StreamHandler(sys.stdout)
console_output_handler_with_multiple_params.setFormatter(
    MultipleParamsFormatter('%(asctime)s - %(message2)s - %(name)s - %(levelname)s - %(message)s'))
